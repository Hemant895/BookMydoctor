...
function DoctorList({ doctorList,heading='Popular Doctors' }) {
  return (
    <div className="px-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-7 text-center p-5">
        { doctorList.length>0 ?
          doctorList.map((item, index) => (
            <div className="border-[1px] rounded-lg p-3 hover:border-primary hover:scale-105 transition-all ease-in-out shadow-lg" key={index}>
              ...
              <div className="mt-3 items-baseline flex flex-col gap-2">
                ...
                <h2 className="p-2 px-3 border-[1px] border-primary rounded-full w-full text-center text-[15px] mt-2 cursor-pointer hover:bg-primary hover:text-white text-primary">Book Now</h2>
              </div>
            </div>
          ))
        :
        ...
        [1,2,3,4,5,6].map((item,index)=>(
                <div className=" h-[220px] bg-slate-200 rounded-lg animate-pulse ">
                </div>
